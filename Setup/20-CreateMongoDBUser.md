...
```sh
     Syntax:- mongosh --port <PORT> --authenticationDatabase "<database_name_where_user_stored>" -u "<username>" -p "<password>"
     Example:- mongosh --port 27017 --authenticationDatabase "admin" -u "superuser" -p "HyGuys"
```
- To Make Connection use:
```sh
     Syntax:- mongodb://<username>:<password>@<IP_Address>:<PORT>/<database_name>
     Example:- mongodb://superuser:HyGuys@216.32.44.12:27017/blogdb
```
...
```sh
     ...
     - If Authorization Enable and have Superuser (Recommended)
          mongosh --port 27017 --authenticationDatabase "admin" -u "superuser" -p "HyGuys"
```
- Show database
```sh
     show dbs
...
```sh
     ...
     Example:- db.createUser({user:"abhi", pwd:passwordPrompt(), roles:[{role:"readWrite", db:"blogdb"}]})
```
- Verify Users
```sh
     show users
...
```sh
     Syntax:- mongosh --port <PORT> --authenticationDatabase "<database_name_where_user_stored>" -u "<username>" -p "<password>"
     Example:- mongosh --port 27017 --authenticationDatabase "blogdb" -u "abhi" -p "HyGuys"
```
- To Make Connection use:
```sh
     Syntax:- mongodb://<username>:<password>@<IP_Address>:<PORT>/<database_name>
     Example:- mongodb://abhi:HyGuys@216.32.44.12:27017/blogdb
```